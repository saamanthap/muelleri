...
- Female-specific heterozygosity (ALL females and no males are heterozygous)
- Female-specific nucleotide (males have no data)
- Fixed divergence (males and females are homozygous for different nucleotides)

**Category -1** has three types of positions:
- Male-specific SNP
- Male-specific heterozygosity
- Male-specific heterozygosity 

The output is a tab-delimited text file. It contains the chromosome name, position, type of position, category, and the number of males and females for which there were genotypes. Example:
```
CHR     PS      TYPE    CATEGORY        n_FEM   n_MALE
Chr4L   40912   Fixed_divergence        1       7.0     3.0
Chr4L   40913   Fixed_divergence        1       7.0     3.0
Chr4L   40919   Fixed_divergence        1       7.0     3.0
Chr4L   41865   Fixed_divergence        1       7.0     3.0
Chr4L   43409   Fixed_divergence        1       7.0     3.0
Chr4L   43415   Fixed_divergence        1       7.0     3.0
Chr4L   43421   Fixed_divergence        1       7.0     3.0
Chr4L   43470   Fixed_divergence        1       7.0     3.0
Chr4L   43496   Fixed_divergence        1       7.0     3.0
```
To run the script, create a virtual python environment in your home directory where you can install python modules:
```
cd 
...
```
All modules used in the script are included in python, except for pysam, which you'll have to install. With the virtual environment activated, use the command:
```
pip install pysam
```
When you run the script, make sure to load the correct python module and activate your virtual environment first. Then run like this:
```
python find_het_positions.py vcf_file sex_string output_file proportion
```
**vcf_file:** input vcf. gzipped is okay  
**sex_string:** 0=male, 1=female, 2=ignore. Must be in the same order as the samples in the vcf. Example: 1001111010   
**output_file:** name/path to the output file  
**proportion:** minimum proportion of males or females that must be heterozygous in order for a position to qualify as a sex-specific SNP
...
#the virtual environment should already have numpy and pysam - all other libraries i used are standard

#first i'm going to import useful libraries
import sys #allows me to exit on errors
import argparse #allows me to use command-line arguments
...
def main():
    #i start by setting up the parser object which will control all the command-line arguments. i add a help message that can be printed out if the script is run with the --help flag
